...
export async function POST(request: Request) {
  ...
  try {
    ...
    const formData = await request.formData();
      ...
      const sanitizedBeforeName = sanitizeFileName(beforeFile.name);
      const sanitizedAfterName = sanitizeFileName(afterFile.name);

      // Upload before image
      const beforePath = `${user.id}/before-${Date.now()}-${sanitizedBeforeName}`;
      const { error: beforeError } = await supabase.storage
        .from('images')
        .upload(beforePath, beforeFile);

      if (beforeError) {
        console.error('Before image upload failed:', {
          userId: user.id,
      ...
      const afterPath = `${user.id}/after-${Date.now()}-${sanitizedAfterName}`;
  ...
  console.error('Unexpected error during post creation:', {
    userId: user.id,
